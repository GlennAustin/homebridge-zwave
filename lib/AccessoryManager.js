...
class AccessoryManager {
  ...
  initializeAccessory (accessoryConfig, accessory) {
    ...
    this._zwave.onNodeReady(zwaveNodeId, zwaveNode => {
      ...
      if (this._whenNodeReady != null) {
        this._whenNodeReady(zwaveNodeId, zwaveNode)
      }
    })
  }
  ...
  _configureOutletService () {
    ...
    if (this._accessoryConfig.pollForStatus) {
      this._whenNodeReady = (zwaveNodeId, zwaveNode) => {
        const nodeValueId = this._zwave.generateNodeValueId({
          nodeId: this._accessoryConfig.zwaveNodeId,
          commandClass: COMMAND_CLASS_SWITCH_MULTILEVEL,
          valueIndex: SWITCH_MULTILEVEL_INDEX_LEVEL,
        })
        this._log(`${this._accessoryConfig.displayName} setting up polling`)

        this._zwave.enablePoll(nodeValueId, 1)
      }
    }
  }
   ...
   */
  _configureLightService () {
    const service = this._getService(this._hapService.Lightbulb)
    const onCharacteristic = service.getCharacteristic(this._hapCharacteristic.On)
    const brightnessCharacteristic = service.getCharacteristic(this._hapCharacteristic.Brightness)

    // Setup handlers for when the value for the "Brightness" characteristic is requested / updated by HomeKit
    const onCharacteristicName = 'On'
    ...
    onCharacteristic
      .on('get', done => {      
          ...
          this._log(`${accessoryDisplayName} "${onCharacteristicName}" characteristic value requested`)
          ...
          const zwaveNodeValueResolver = this._makeZwaveNodeValueResolver(brightnessCharacteristicZwaveNodeValueCriteria)
          const value = (zwaveNodeValueResolver() != 0) ? true : false
        
          this._log(`${accessoryDisplayName} "${onCharacteristicName}" characteristic is ${value}`)
      ...
      .on('set', (value, done) => {
          ...
          this._log(`${accessoryDisplayName} "${onCharacteristicName}" value updating to: ${value}`)
    ...
    brightnessCharacteristic
      .on('get', this._makeCharacteristicGetterForZwaveNodeValue(
				  ...
				  this._makeZwaveNodeValueResolver(brightnessCharacteristicZwaveNodeValueCriteria)
			  )
			)
      .on('set', this._makeCharacteristicSetterForZwaveNodeValue2(
          brightnessCharacteristicName,
          brightnessCharacteristicZwaveNodeValueCriteria
				)
			)

    // Setup handler for when the corresponding ZWave node values are updated outside of HomeKit
    const accessoryDisplayName = this._accessoryConfig.displayName
    const zwaveNodeId = this._accessoryConfig.zwaveNodeId
    const nodeValueId = this._zwave.generateNodeValueId({
      nodeId: this._accessoryConfig.zwaveNodeId,
    ...
    this._zwave.onNodeValueChanged(nodeValueId, value => {
      if (this._inProcess) {
				if (this._currentValue != value) {
					this._log(`${accessoryDisplayName} "Brightness" ${value} going to ${this._currentValue}`)
// 					brightnessCharacteristic.updateValue(value)
// 					onCharacteristic.updateValue((value != 0) ? true : false)
					this._zwave.updateNodeValueById(nodeValueId, this._currentValue)
				} else {
					this._log(`${accessoryDisplayName} "Brightness" characteristic value updated to ${value}`)
					this._inProcess = false
					brightnessCharacteristic.updateValue(value)
					onCharacteristic.updateValue((value != 0) ? true : false)
				}
        ...
        this._log(`${accessoryDisplayName} "Brightness" characteristic value updated to ${value} outside of HomeKit`)
      }
    })
  ...
  _makeCharacteristicGetterForZwaveNodeValue (
    ...
    return done => {
      ...
      const value = valueFilter(zwaveNodeValueResolver())

      this._log(`${accessoryDisplayName} "${characteristicName}" characteristic is ${value}`)
  ...
  _makeCharacteristicSetterForZwaveNodeValue (
    ...
    return (value, done) => {
      const accessoryDisplayName = this._accessoryConfig.displayName
      const zwaveNodeId = this._accessoryConfig.zwaveNodeId
      const newValue = Math.min(99.0, valueFilter(value))
      const nodeValueId = this._zwave.generateNodeValueId({
        nodeId: zwaveNodeId,
        commandClass,
        valueIndex
   ...
   * @returns {Function}
   */
  _makeCharacteristicSetterForZwaveNodeValue2 (
    characteristicName,
    { commandClass, valueIndex },
    valueFilter = v => v
  ) {
    return (value, done) => {
      ...
      this._log(`${accessoryDisplayName} "${characteristicName}" value updating to: ${value}`)
