  ...
  class ZWavePlatform {
    ...
    _validateConfig () {
      const accessoryConfigSchema = Joi.object().keys({
        ...
        pollForStatus: Joi.boolean().optional(),
        homekitCategory: Joi.string().valid(['Outlet', 'Sensor', 'Switch', 'Light']).required(),
      ...
      const configSchema = Joi.object().keys({
        ...
        pollIntervalSec: Joi.number().optional(),
    ...
    _initPlatform () {
      ...
      this._zwave.init((err) => {
        ...
        if (this._config.pollIntervalSec != null && this._config.pollIntervalSec > 0) {
          this._zwave.setPollInterval(this._config.pollIntervalSec * 1000)
        }
      })
    }
